...
import { usePersistForm } from "@/hooks/form/use-persist-form";
import { usePrioritizedSort } from "@/hooks/search/use-prioritized-sort";
...
export const HomeForm = ({
  ...
  provinces: Province[];
  handleSubmit: (data: { province: string; district: string }) => void;
}) => {
  const form = usePersistForm<{ province: string; district: string }>(
    {
      defaultValues: {
        province: "",
        district: "",
  ...
  const districtsOptions =
    provinces
      .find(
        (province) => province.name.toString() === form.getValues("province")
      )
  ...
  return (
    <div
      className="flex flex-col md:flex-row items-end gap-4 p-6 transition-all duration-500
    ...
    >
      ...
      <div className="w-full space-y-2">
        <label
          className="text-[11px] font-bold tracking-[0.1em] ml-1 uppercase
